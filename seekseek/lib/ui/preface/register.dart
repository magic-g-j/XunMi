...
class _RegisterPageState extends State<RegisterPage> {
  ...
  register() async {
    try {
      ...
      Response response;
      Dio dio = new Dio();
      if(_name==null || _phone==null || _password==null || _sex==null || _words==null){
        ToastHelper.showToast(context, "注册失败：各注册信息不能为空");
      }
      ...
      if (response.statusCode == 200 ) {
        ...
        print(userId);
        if(userId==-9 && (_code!=null || _code!="")){
          ToastHelper.showToast(context, "注册失败：验证码错误");
        }
        else if(userId==-8){
          ToastHelper.showToast(context, "注册失败：该手机号已被占用");
        }
        else{
...
//      onSaved
    ...
    final  tip = new Center(
      child: new Text('我的-个人信息 可进行修改',style: new TextStyle(color: Colors.grey),),
    );
